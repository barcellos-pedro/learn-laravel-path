                ...
                <div class="hidden md:block">
                    <div class="ml-10 flex items-baseline space-x-4">
                        ...
                        <?php if ($_SESSION['user'] ?? false) : ?>
                            <a href="/notes" class="<?= activeLink("/notes") ?> hover:text-white rounded-md px-3 py-2 text-sm font-medium">Notes</a>
                        <?php endif; ?>
                ...
                <div class="ml-4 flex items-center md:ml-6">
                    ...
                    <div class="flex items-center gap-3 ml-3">
                        <button type="button" class="flex max-w-xs items-center rounded-full bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
                            <?php if ($_SESSION['user'] ?? false) : ?>
                                <img class="h-8 w-8 rounded-full" src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" alt="image of generic person face">
                                <form method="POST" action="/session">
                                    <input type="hidden" name="_method" aria-hidden="true" value="DELETE" />
                                    <button type="submit" class="text-gray-300 text-sm font-medium">Log out</button>
                                </form>
                            <?php else : ?>
                                <div class="flex gap-3 items-center">
                                    <a href="/register" class="<?= activeLink("/register") ?> hover:text-white rounded-md px-3 py-2 text-sm font-medium">Register</a>
                                    <a href="/login" class="<?= activeLink("/login") ?> hover:text-white rounded-md px-3 py-2 text-sm font-medium">Log
                                        in</a>
                                </div>
                            <?php endif; ?>
                        </button>
    ...
    <div class="md:hidden" id="mobile-menu">
        ...
        <div class="border-t border-gray-700 pb-3 pt-4">
            ...
            <div class="mt-3 space-y-1 px-2">
                ...
                <a href="/notes" class="block rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white">Notes</a>
